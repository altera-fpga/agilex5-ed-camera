...
## Single camera connected to MIPI1 configuration produces no camera output

**Issue** <br/>
After boot, a single camera module connected to MIPI1 produces no camera output
on the monitor. The monitor only displays a black screen with the Altera® logo.

**Workaround** <br/>
Using the Web GUI, on the `Input Config` Tab, use the `Input Source` Tile to
toggle between `Camera` and `Input TPG`, and back again to `Camera`. This
restores the output on the Monitor.
