...
![top-block-diagram.](../camera_4k/images/ISP/top-block-diagram.png)
...
![isp_ingest.](../camera_4k/images/ISP/isp_ingest.png)
...
the raw sensor image before any ISP processing has taken place. The function
uses a VVP Broadcaster IP, a VVP Color Plane Manager IP, and a VVP Video Frame
Writer IP which is connected to an external FPGA DDR4 SDRAM memory (via an
EMIF).

The Broadcaster is used to feed the Video Frame Writer with the raw sensor
...
This Instance is used to facilitate the following 3D LUT IP where the available
...
![output_processing.](../camera_4k/images/ISP/output_processing.png)
...
the final output image. The function uses a VVP Broadcaster IP and a VVP Video
Frame Writer IP which is connected to an external FPGA DDR4 SDRAM memory (via
an EMIF).

The Broadcaster is used to feed the Video Frame Writer with the output image.
The Video Frame Writer IP operates in a single shot mode on RGB 10-bit images.
