...
# 4Kp60 Multi-Sensor HDR Camera Solution System Example Design for Agilex™ 5 Devices - Known Issues

## Intermittent - no monitor output

**Issue** <br/>
Intermittently, after boot, there may be no output or just a single side output
on the monitor.
...
* Reboot.
* Hot plugging the DP Cable.
* Toggling source input on the monitor.
* Using the Web GUI, on the `Output Config` Tab, use the `Output` Tile to
  toggle between `ISP` and `Colorbars TPG`, and back again to `ISP`.
* Using the Web GUI, on the `Pipeline Stitch` Tab, use the Tile reset button
  located in the top right of the `Stitch` Tile.
* Use a native 4k or Full HD (1080p) resolution monitor.

## Stitch horizontal offset defaults to a theoretical infinity alignment

**Issue** <br/>
The stitch horizontal offset, defaults to a theoretical alignment as if the
view were at infinity. The normal use case requires the x-offset to be adjusted
for the actual scene.

**Workaround** <br/>
Using the Web GUI, on the `Pipeline Stitch` Tab, use the `Stitch` Tile and
the `Offset X` control to adjust the horizontal offset for the actual scene.
