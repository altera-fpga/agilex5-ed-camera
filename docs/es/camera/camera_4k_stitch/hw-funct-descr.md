...
# 4Kp60 Multi-Sensor Camera Stitch Solution System Example Design for Agilex™ 5 Devices - Hardware Functional Description

The hardware design for the 4Kp60 Multi-Sensor Camera Stitch Solution System
...
![top_block_diagram.](../camera_4k_stitch/images/HW/top_block_diagram.png)
...
* The OCS subsystem (readable by the Software Application) is a ROM describing
  ...
  memory map.
* MIPI_In, ISP_In, Stitch_ISP, Stitch_Out, and VID_Out subsystems are the IP
  related to camera image ingress and Image Signal Processing (ISP). The
  MIPI_In subsystem includes the D-PHY for interfacing the Framos MIPI
  connectors on the Modular Development Kit Carrier Board to the FPGA.
...
Example Design (as instantiated from the [AGX_5E_Modular_Devkit_ISP_Stitch_RD.xml](https://github.com/altera-fpga/agilex5-ed-camera/blob/rel/26.1/AGX_5E_Altera_Modular_Dk_ISP_designs/AGX_5E_Modular_Devkit_ISP_Stitch_RD.xml)
file) are described in greater detail below.

<br/>
<center markdown="1">

![pd_top](../camera_4k_stitch/images/HW/pd_top.png)
...
* [Stitch_ISP Subsystem](#stitch_isp-subsystem)
* [Stitch_Out Subsystem](#stitch_out-subsystem)
...
![pd_board](../camera_4k_stitch/images/HW/pd_board.png)
...
![pd_clocks](../camera_4k_stitch/images/HW/pd_clocks.png)
...
![pd_hps](../camera_4k_stitch/images/HW/pd_hps.png)
...
![pd_hps_block_diagram](../camera_4k_stitch/images/HW/pd_hps_block_diagram.png)
...
to the FPGA DDR4 SDRAMs and can be used to offload memory copy functions
...
| 0x4010_0000 | 0x4017_FFFF | Stitch_ISP | Stitch ISP 1 subsystem IP |
| 0x4020_0000 | 0x4027_FFFF | Stitch_ISP | Stitch ISP 2 subsystem IP |
| 0x4030_0000 | 0x4030_0FFF | ISP_In | ISP Input subsystem IP |
| 0x4040_0000 | 0x4040_3FFF | MIPI_In | MIPI Input subsystem IP |
| 0x4050_0000 | 0x4050_3FFF | Vid_Out | Video Output subsystem IP |
| 0x4060_0000 | 0x4060_0FFF | Stitch_Out | Stitch Output subsystem IP |
...
![pd_ocs](../camera_4k_stitch/images/HW/pd_ocs.png)
...
![pd_mipi_in](../camera_4k_stitch/images/HW/pd_mipi_in.png)
...
![pd_mipi_in_block_diagram](../camera_4k_stitch/images/HW/pd_mipi_in_block_diagram.png)
...
AXI4-S Full to VVP AXI4-S Lite.
<br/>
<br/>
...
![pd_isp_in](../camera_4k_stitch/images/HW/pd_isp_in.png)
...
![pd_isp_in_block_diagram](../camera_4k_stitch/images/HW/pd_isp_in_block_diagram.png)
...
For the 4Kp60 Multi-Sensor Camera Stitch Solution System Example Design for Agilex™ 5 Devices, the Switch has dual outputs.
<br/>
<br/>
...
| Address Start | Address End | Module | Description |
|:----:|:----:| ---- | ---- |
| 0x4030_0000 | 0x4030_01FF | VVP Throttle IP | Test Pattern Generator Throttle |
...
### **Stitch_ISP Subsystem**

The Stitch_ISP subsystem (ISP Subsystem) is the main image processing pipeline.
Reference should be made to the [ISP Functional Description.](./isp-funct-descr.md)
when reading this section in order to understand the exact IP function.
...
![pd_isp](../camera_4k_stitch/images/HW/pd_isp.png)
...
![pd_isp_block_diagram](../camera_4k_stitch/images/HW/pd_isp_block_diagram.png)
...
The VVP Warp IP is then used to correct for lens distortion and to project the
image onto a cylindrical canvas for the stitching blend that follows. The Warp
is configured to support double bounce mode and uses the external 8GB FPGA DDR4
SDRAM. 128MB is allocated for the transform coefficients (which the HPS
Software writes) while the remainder is used as frame buffers. The Warp
interfaces to an Address Span Extender which provides the 512MB window into the
EMIF.
<br/>
<br/>
...
every video line. The pixel width of the blend and the blend profile values are
programmed by the HPS SW.
<br/>
<br/>

The Stitch_ISP output is fed to the [Stitch_Out Subsystem](#stitch_out-subsystem).
...
| 0x4020_8000 | 0x4020_83FF | VVP Scaler IP | H Down Scaler |
| 0x4020_8400 | 0x4020_87FF | VVP Scaler IP | V Down Scaler |
...
| 0x4020_9A00 | 0x4020_9BFF | VVP CSC IP | Color Correction Matrix |
| 0x4020_A600 | 0x4020_A6FF | Alpha Channel IP | Alpha Channel |
| 0x4020_A800 | 0x4020_A8FF | ROI IP | Region of Interest |
| 0x4020_E000 | 0x4020_EFFF | VVP HS IP | Histogram Statistics |
...
contains a VVP 3D LUT IP. The 3D LUT IP is configured for 10-bit input/output
with a LUT size of 17 cubed and 10-bit color depth. The LUT can be placed in
bypass mode when not required.
<br/>
<br/>

The VVP TMO IP implements a ROI based tone mapping algorithm to improve the
visibility of latent image detail across areas of the image.
<br/>
<br/>
...
The USM output is fed to the [VID_Out Subsystem](#vid_out-subsystem).
<br/>
<br/>
...
**HPS Subsystem to Stitch_Out Subsystem Memory Map**

| Address Start | Address End | Module | Description |
|:----:|:----:| ---- | ---- |
| 0x4060_0000 | 0x4060_01FF | VVP TPG IP | Test Pattern Generator |
| 0x4060_0400 | 0x4060_07FF | VVP Mixer IP | Mixer |
| 0x4060_0800 | 0x4060_09FF | VVP 3D-LUT IP | 3D LUT |
| 0x4060_0A00 | 0x4060_0BFF | VVP TMO IP | Tone Mapping Operator |
| 0x4060_0C00 | 0x4060_0DFF | VVP USM IP | Un-Sharp Mask Filter |
...
![pd_emif](../camera_4k_stitch/images/HW/pd_emif.png)
...
|:----:|:----:|:----:| ---- | ---- |
| 0x0_0000_0000 | 0x0_17FF_FFFF | 384MB | Stitch_ISP 0 subsystem | Warp Buffers |
| 0x0_1800_0000 | 0x0_1FFF_FFFF | 128MB | Stitch_ISP 0 subsystem | Warp Coefficients |
...
| Address Start | Address End | Size | Module | Description |
|:----:|:----:|:----:| ---- | ---- |
| 0x0_0000_0000 | 0x0_17FF_FFFF | 384MB | Stitch_ISP 1 subsystem | Warp Buffers |
| 0x0_1800_0000 | 0x0_1FFF_FFFF | 128MB | Stitch_ISP 1 subsystem | Warp Coefficients |
| 0x0_2000_0000 | 0x1_FFFF_FFFF | 7.5GB | - | Unused |
...
![pd_vid_out](../camera_4k_stitch/images/HW/pd_vid_out.png)
...
![pd_vid_out_block_diagram](../camera_4k_stitch/images/HW/pd_vid_out_block_diagram.png)
...
![pd_dp_tx](../camera_4k_stitch/images/HW/pd_dp_tx.png)
...
![qpds_flow_summary](../camera_4k_stitch/images/HW/qpds_flow_summary.png)
...
![qpds_hierarchy](../camera_4k_stitch/images/HW/qpds_hierarchy.png)
...
![qpds_fmax](../camera_4k_stitch/images/HW/qpds_fmax.png)
...
![qpds_design_closure](../camera_4k_stitch/images/HW/qpds_design_closure.png)
