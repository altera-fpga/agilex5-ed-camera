...
# 4Kp60 Multi-Sensor Camera Stitch Solution System Example Design for Agilex™ 5 Devices - Features
...
* Input selection switch with dual outputs

* Dual Frontend video pipelines (CFA image):
  * (Optional) Clipper IP.
  * ISP IP: (Optional) Black Level Statistics (BLS), Defective Pixel Correction
  (DPC), Adaptive Noise Reduction (ANR), Black Level Correction (BLC), Vignette
  Correction (VC), White Balance Statistics (WBS), White Balance Correction
  (WBC), and Demosaic (DMS).

* Dual Frontend video pipelines (RGB image):
  * Histogram Statistics (HS) IP.
  * Color Correction Matrix (CCM) IP.
  * Region Of Interest (ROI) IP.
  * Scaler IP.
  * Warp IP.
  * Alpha Channel IP (on 1 pipeline).

* Stitch Backend video pipeline to stitch images together (RGB image):
  * Mixer IP:
    * TPG for uniform background.
    * ISP Stitch Output overlay.
    * ISP Stitch with Alpha Output overlay.
  * 3D LUT IP.
...
* Backend video pipeline (RGB image):
  * Mixer IP:
    ...
    * Stitch Output overlay.
    * HPS OSD with Alpha overlay.
      * HPS DDR OSD DMA using Frame Reader IP.
      * Upscale using Scaler IP.
  * 1D LUT IP – for OETF operations such as Gamma, HLG, and PQ.

* Up to 4Kp60 10-bit standard Multi-rate DisplayPort output:
  * Selects best resolution and color depth supported by the connected Monitor.
  Supported modes:
    * 720p60, 1080p60, 4Kp30, 8-bit and 10-bit color depth.
    * 4Kp60 8-bit color depth.
  * 5120*1440p34 and 3840x1080p60 non-standard Monitor support (specific Monitor
  compatibility cannot be guaranteed).
  * Note that the input image is resized such that the stitched output fills
  the horizontal resolution of the connected monitor therefore maintaining the
  correct aspect ratio. So for a 16:9 monitor, the design will display a letter
  box image, while a 32:9 monitor will display a slightly cropped image but
