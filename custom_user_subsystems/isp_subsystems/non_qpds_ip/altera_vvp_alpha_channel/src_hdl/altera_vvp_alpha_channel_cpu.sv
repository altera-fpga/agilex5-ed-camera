 ...
 * ##################################################################################
 *
 * Module: altera_vvp_alpha_channel_cpu
 *
 * Description: CPU interface for Alpha Channel
 *
 * ##################################################################################
*/
...
module altera_vvp_alpha_channel_cpu #(
  ...
  localparam C_REG_LUT_WRITE    = C_CPU_OFFSET + 2;
  ...
  output  logic [C_LUT_ADDR_BITS-1:0]     r_lut_write_addr;
  output  logic                           r_lut_write_en;
  output  logic [C_LUT_DATA_BITS-1:0]     r_lut_write_data;
  ...
  logic [31:0]                r_cpu_lut_write;

  logic [31:0]                r_vid_control_meta;
  logic [31:0]                r_vid_control_safe;
  logic [31:0]                r_vid_control_safe_d1;

  logic [31:0]                r_lut_write_meta;
  logic [31:0]                r_lut_write_safe;
  logic [31:0]                r_lut_write_safe_d1;
  logic                       r_lut_write_primed;
...
//-----------------------------------------------------------------
  ...
  always_ff @(posedge agent_clock) begin : a_make_cpu_if
    ...
    if (C_USE_CPU) begin
      ...
      case (r_av_address)
        ...
        C_REG_LUT_WRITE     : begin
                                nb_local_reg32          = r_cpu_lut_write;
                                av_readdata             <= nb_local_reg32;
                                t_cpu_write(r_av_write , r_av_byteenable , r_av_writedata , nb_local_reg32);
                                r_cpu_lut_write           <= nb_local_reg32;
                              end
  ...
  always_ff @(posedge main_clock) begin : a_cdc_cpu_if
    ...
    r_lut_write_meta      <= r_cpu_lut_write;
    r_lut_write_safe      <= r_lut_write_meta;
    r_lut_write_safe_d1   <= r_lut_write_safe;

    if (r_lut_write_safe == r_lut_write_safe_d1) begin
      r_lut_write_primed  <= 1'b0;
      r_lut_write_addr    <= r_lut_write_safe_d1[C_LUT_ADDR_BITS-1:0];
      r_lut_write_en      <= r_lut_write_primed;
      r_lut_write_data    <= r_lut_write_safe_d1[C_LUT_DATA_BITS+11:12];
    end else begin
      r_lut_write_primed   <= 1'b1;
    end
    ...
    if (main_reset) begin
      ...
      r_vid_control           <= 32'b0;
      r_lut_write_primed      <= 1'b0;
      r_lut_write_addr        <= {C_LUT_ADDR_BITS{1'b0}};
      r_lut_write_en          <= 1'b0;
      r_lut_write_data        <= {C_LUT_DATA_BITS{1'b0}};
    end
