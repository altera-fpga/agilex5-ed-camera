 ...
 * ##################################################################################
 *
 * Module: altera_vvp_alpha_channel
 *
 * Description: Alpha Channel Core
 *
 * ##################################################################################
*/
...
module altera_vvp_alpha_channel_core #(
  ...
  parameter BPS                                     = 10, // standard VVP name - Only 10bps supported
  parameter C_LUT_ADDR_BITS                         = 10  // Only 10 supported (1024 entries BPS*PIP width)
) (
  // CPU Clock and Reset
  agent_clock,
  agent_reset,
  ...
  localparam C_LUT_DATA_BITS        = BPS*PIXELS_IN_PARALLEL;
  ...
  logic [C_LUT_ADDR_BITS-1:0]     r_lut_write_addr;
  logic                           r_lut_write_en;
  logic [C_LUT_DATA_BITS-1:0]     r_lut_write_data;


  //  CPU  //
  altera_vvp_alpha_channel_cpu #(
      .C_CPU_OFFSET             (C_CPU_OFFSET) ,
      .C_USE_CPU                (RUNTIME_CONTROL),
      .C_LUT_ADDR_BITS          (C_LUT_ADDR_BITS),
      .C_LUT_DATA_BITS          (C_LUT_DATA_BITS) )
  u_altera_vvp_alpha_channel_cpu (
      ...
      .r_lut_write_addr       (r_lut_write_addr),
      .r_lut_write_en         (r_lut_write_en),
      .r_lut_write_data       (r_lut_write_data) );


  //  Alpha Channel Generator  //
  altera_vvp_alpha_channel_gen #(
      .NUMBER_OF_COLOR_PLANES   (NUMBER_OF_COLOR_PLANES),
      .PIXELS_IN_PARALLEL       (PIXELS_IN_PARALLEL),
      .BPS                      (BPS),
      .C_LUT_ADDR_BITS          (C_LUT_ADDR_BITS),
      .C_LUT_DATA_BITS          (C_LUT_DATA_BITS) )
  u_altera_vvp_alpha_channel_gen (
      .main_clock             (main_clock),
      .main_reset             (main_reset),
