...
class TabbedUiControls
{
    public:
        TabbedUiControls(std::function<std::shared_ptr<TopLevelUiTab>(const std::string&, BooleanControlCB, const std::string&)> appCreateUiTabCB,
                        std::shared_ptr<UdxVvpIspPipeline> spPipeline, bool debugMode, bool powerUser, std::function<void(uint32_t)> warpCaptureCB = nullptr);
        ~TabbedUiControls();
        TabbedUiControls(const TabbedUiControls& other) = delete;
        TabbedUiControls& operator=(const TabbedUiControls& other) = delete;
        TabbedUiControls(TabbedUiControls&& other) = delete;
        TabbedUiControls& operator=(TabbedUiControls&& other) = delete;
        ...
        std::shared_ptr<TopLevelUiTab> _spOutputConfigTab;
            std::shared_ptr<WarpFullControls> _spWarpControls;
    ...
    private:
        ...
        CameraUiControlsPtr _spActiveCameraUiControls;

        float _backedUpCameraGain = 1.0f;
        bool _autoExpEnableState = false;
        std::size_t _aeTaskHandle;
        std::size_t _awbTaskHandle;
        SwApi::HistogramStats::HistogramSubscriptionId _histogramPreviewSubscriptionId = 0;
        std::weak_ptr<SwApi::HistogramStats> _wpHistogramPreviewSource;
