...
MultiChannelAutoExposureControls::MultiChannelAutoExposureControls(const std::shared_ptr<SwApi::MultiChannelAutoExposure>& spMultiChannelAutoExposure)
: _spMultiChannelAutoExposure(spMultiChannelAutoExposure)
, _maxDeviationFromNominal(0)
{
    uint32_t numChannels = _spMultiChannelAutoExposure->GetNumberOfChannels();
...
std::vector<std::shared_ptr<UiControlContainer>> MultiChannelAutoExposureControls::AddUiElements() 
{
    auto spContainer = std::make_shared<UiControlContainer>("Multi-Channel Auto Exposure", GetSettingsSectionName());
        ...
        for(uint32_t pipeline_index = 0; pipeline_index < numChannels; pipeline_index++)
        {
            auto contributionCB = [this, pipeline_index, numChannels](uint32_t clientID, int32_t& val)
            {
                ...
                for(uint32_t index = 0; index < numChannels; index++)
                {
                    ...
                    _spMultiChannelAutoExposure->SetChannelContributionToNominal(index, (float)_contributionToNominal[index]/100.0);
    ...
    auto deviationCB = [this](uint32_t clientID, int32_t& val)
    {
        ...
        _spMultiChannelAutoExposure->SetMaxDeviationFromNormal((float)_maxDeviationFromNominal/100.0);
    };
