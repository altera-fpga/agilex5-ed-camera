...
public:
  ...
  int get_device_handle(const char *name)
  {
    for(auto& it : handle_to_dev)
    {
...
int AOCL_MMD_CALL aocl_mmd_open(const char *name) {
  int handle = _gDeviceMapManager.get_device_handle(name);
  if(handle == -1)
  {
    handle = _gDeviceMapManager.add_device(name);
  }
  return handle;
}
...
int AOCL_MMD_CALL
aocl_mmd_stream_controller_msg_send_interrupt(int handle)
{
  mmd_device_ptr spDevice = _gDeviceMapManager.get_device(handle);
  if( nullptr == spDevice ) {
    return FAILURE;
  }
  return spDevice->stream_controller_msg_send_interrupt();
}

int AOCL_MMD_CALL 
aocl_mmd_stream_controller_msg_receive_interrupt(int handle)
{
  mmd_device_ptr spDevice = _gDeviceMapManager.get_device(handle);
  if( nullptr == spDevice ) {
    return FAILURE;
  }
  return spDevice->stream_controller_msg_receive_interrupt();
}
...
AOCL_MMD_CALL int dla_mmd_stream_controller_msg_send_interrupt(int handle, int instance)
{
  return aocl_mmd_stream_controller_msg_send_interrupt(handle);
}

AOCL_MMD_CALL int dla_mmd_stream_controller_msg_receive_interrupt(int handle, int instance)
{
  return aocl_mmd_stream_controller_msg_receive_interrupt(handle);
}
