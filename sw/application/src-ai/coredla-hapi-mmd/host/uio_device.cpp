...
//////////////////////////////////////////////////////////////
uio_device::uio_device(std::string &name, const int mmd_handle, const bool bEnableIRQ, const bool bAsyncIRQ)
...
#ifndef RUNTIME_POLLING
    if( bEnableIRQ ) {
        _spInterrupt = std::make_shared<uio_interrupt>(name, _fd, _mmd_handle, bAsyncIRQ);
...
bool uio_device::wait_irq() {
#ifndef RUNTIME_POLLING
    if( _spInterrupt ) {
        return _spInterrupt->wait_irq();
    }
#endif
    return FAILURE;
}
...
///////////////////////////////////////////////////////////////////////////////////
uio_interrupt::uio_interrupt(std::string &name, const int fd, const int mmd_handle, const bool async)
: _name(name), _device_fd(fd), _mmd_handle(mmd_handle), _async(async) {
    if( is_irq_available() ) {
...
bool uio_interrupt::wait_irq() {
    ...
    if (res < 0) {
        ...
        if (bytes_read > 0) {
            return false; // We've been asked to shutdown
        } else {
