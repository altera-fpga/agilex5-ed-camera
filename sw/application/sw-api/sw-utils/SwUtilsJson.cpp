...
namespace SwUtils
{
    ...
    {
        auto spJson = std::make_shared<Json>(filePath);
    ...
    Json::Json(std::filesystem::path filePath)
    {
        _rootObject = std::make_shared<JsonObject>(this);
        _rootObject->_isRootObject = true;
        ...
        if (inputStream.good())
        {
            ...
            std::vector<char> fullData(fileSize);
            inputStream.read(fullData.data(), fileSize);
